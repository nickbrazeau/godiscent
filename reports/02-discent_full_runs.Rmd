...
discdat <- discdat %>% 
  dplyr::mutate(cost = purrr::map_dbl(mod, function(x){tail(x$cost,1)}))
#............................................................
# Helper Functions
#...........................................................
...
avg_disc_dat_Func <- function(dat, filt){
  avgM <- dat %>% 
    dplyr::filter(overallmodname == filt) %>% 
    dplyr::summarise( avgM = mean(avgM)) %>% 
    dplyr::pull(avgM)
p1 <- dat %>% 
  dplyr::filter(overallmodname == filt) %>% 
  ggplot() +
  geom_point(aes(x = deme1longnum, y = deme1latnum, color = avgF),
             size = 3) +
  theme_minimal() +
  scale_color_viridis() +
  theme(axis.title = element_blank()) +
  labs(title = paste("Average Fs and M for Model", filt),
       caption = paste("Average M: ", round(avgM, digits = 2)))

sdM <- dat %>% 
  dplyr::filter(overallmodname == filt) %>% 
  dplyr::summarise( sdM = mean(sdM)) %>% 
  dplyr::pull(sdM)
p2 <- dat %>% 
  dplyr::filter(overallmodname == filt) %>% 
  ggplot() +
  geom_point(aes(x = deme1longnum, y = deme1latnum, color = sdF),
             size = 3) +
  theme_minimal() +
  scale_color_viridis() +
  theme(axis.title = element_blank()) +
  labs(title = paste("St Dev Fs and M for Model", filt),
       caption = paste("St Dev M: ", round(sdM, digits = 2)))
...
#++++++++++++++++++++++++++++++++++++++++++
#### Part 1: Visualize Individual Results     ####
#++++++++++++++++++++++++++++++++++++++++++
...
#++++++++++++++++++++++++++++++++++++++++++
#### Part 2: Visualize Average Results     ####
#++++++++++++++++++++++++++++++++++++++++++
avgdiscdat <- discdat %>% 
  dplyr::mutate( overallmodname = stringr::str_extract(string = modname, pattern = "^(.*?)(?=-rep)") ) %>% 
  dplyr::select(c("overallmodname", "mod")) %>% 
  dplyr::mutate(
    Final_Fis = purrr::map(mod, function(x){x$Final_Fis}),
    Final_m = purrr::map(mod, function(x){x$Final_m}),
    deme_key = purrr::map(mod, function(x){x$deme_key}),
  ) %>% 
  dplyr::select(-c("mod")) %>% 
  tidyr::unnest(cols = c(Final_Fis, Final_m, deme_key)) %>% 
  dplyr::group_by(overallmodname, Deme) %>% 
  dplyr::summarise(
    avgF = mean(Final_Fis),
    avgM = mean(Final_m),
    sdF = sd(Final_Fis),
...
avgdiscdatlocat <- locatdat %>% 
  dplyr::select(c("deme1", "deme1longnum", "deme1latnum")) %>% 
  dplyr::filter(!duplicated(.)) %>% 
  dplyr::left_join(avgdiscdat, ., by = "deme1") %>% 
  dplyr::select(c("overallmodname", "deme1", "deme1longnum", "deme1latnum", "avgF", "avgM", "sdF", "sdM"))
...
- For each five migration regime repetition, the best parameter set (lowest cost) was identified from the search grid (eg 50 unique starting models).
- A second run using this configuration was executed for 100,000 steps to refine estimates.
...
#### Average 
```{r} 
avg_disc_dat_Func(dat = avgdiscdatlocat, filt = "torus")
``` 
...
#### Average 
```{r} 
avg_disc_dat_Func(dat = avgdiscdatlocat, filt = "bound")
``` 
...
#### Average 
```{r} 
avg_disc_dat_Func(dat = avgdiscdatlocat, filt = "dexter")
``` 
...
#### Average 
```{r} 
avg_disc_dat_Func(dat = avgdiscdatlocat, filt = "torus-NeVary")
``` 
...
#### Average 
```{r} 
avg_disc_dat_Func(dat = avgdiscdatlocat, filt = "bound-NeVary")
``` 
